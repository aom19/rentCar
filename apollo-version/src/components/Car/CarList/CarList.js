...
import { CREATE_CAR } from "../../../graphql/Mutations/car";
import { LOAD_CARS } from "../../../graphql/Query/car";

const CarList = (props) => {
  const { bookings, cars } = props;

  const context = useContext(AuthContext);

  // const [createEvent, { error: errorCreateCar, loading: loadingCreateCar }] =
  //   useMutation(CREATE_CAR, { refetchQueries: [LOAD_CARS, "events"] });

  const [createEvent, { error: errorCreateCar, loading: loadingCreateCar }] =
    useMutation(CREATE_CAR, {
      update: (cache, { data }) => {
        const newCar = data?.createEvent;
        cache.evict({
          id: "ROOT_QUERY",
          field: "events",
          args: { newCar },
  ...
  useEffect(() => {
    if (context.token !== null && bookings?.length !== 0 && !context.isAdmin) {
      let result = cars?.filter(
        (o1) => !bookings?.some((o2) => o1._id === o2.event._id)
      );
      console.log("first if authenticated  cars - bookedCars");
      ...
      console.log("2nd if authenticated  Admin allCars ");
      setFilterCars(cars);
    } else if (context?.token === null || bookings?.length === 0) {
      setFilterCars(cars);
      console.log("3 if unauthenticated");
    }
    return function cleanup() {
      setFilterCars(cars);
    };
  }, [cars, bookings, context.token]);
    ...
    createEvent({
      variables: {
        eventInput: {
          ...
          fuel: values.fuel,
          airConditions: `${values.airConditions}`,
          GPS: `${values.GPS}`,
          childSeat: `${values.childSeat}`,
          music: `${values.music}`,
          seatBelts: `${values.seatBelts}`,
          sleepingBed: `${values.sleepingBed}`,
          water: `${values.water}`,
          bluetooth: `${values.bluetooth}`,
          onBoardComputer: `${values.onBoardComputer}`,
          audioInput: `${values.audioInput}`,
          carKit: `${values.carKit}`,
          remoteLocking: `${values.remoteLocking}`,
          climateControl: `${values.climateControl}`,
          price: values.price,
            ...
            <div class="car-wrap rounded ">
              ...
              <div class="text">
                ...
                <p class="d-flex mb-0 d-block">
                  <a
                    class="btn btn-warning py-2 ml-1 text-white"
